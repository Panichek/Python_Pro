...
def find_geo_from_ip(ip_address, service_http='https://app.ipgeolocation.io/ipgeo?=apikey=',
                     key='d6e251aa4f4d4a6aafda61a6e3394cea'):
    ...
    try:
        url = str(service_http) + str(key) + '&ip=' + str(ip_address)
...
df.to_csv("df_saved7_2.csv")

new_df = pd.read_csv("df_saved7_2.csv")
print(new_df.dtypes)
print(new_df)
